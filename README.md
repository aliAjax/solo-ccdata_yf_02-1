...
- **完整撤销/重做**：命令模式，撤销/重做本身也作为新事件广播，跨标签页一致；连续方向键微调自动合并为一步。**并发冲突保护**：每个撤销/重做条目记录操作生效后对象应处的状态（守卫），执行前校验——若对象已被其他标签页更新，该条目过期，丢弃并提示，绝不把较新版本覆盖回旧值。
...
`test/verify.cjs` 端到端覆盖：多标签同步（增/移/编/删）、刷新恢复、重复事件幂等、撤销重做及跨标签同步、PNG 导出（魔数/尺寸/无工具栏）、窄屏与键盘操作（21 项）。
`test/conflict.cjs` 并发撤销冲突：双标签先后移动同一对象后，较早标签的过期撤销被跳过（不覆盖较新位置）、较新标签的撤销/重做正常生效并同步、单标签撤销链不受影响、过期"撤销添加"不删除他人已移动的对象（7 项）。
`test/nudge.cjs` 微调交错时序：A 微调 → B 远端更新 → A 再微调（合并窗口内）时两次微调不合并、较早快照立即失效、撤销保留远端位置；无远端插入时普通连续微调仍合并为一步撤销（6 项）。
`test/editconflict.cjs` 并发文本编辑：编辑器打开期间对象被远端改写时，关闭编辑器不静默提交旧草稿——弹确认框，取消=丢弃草稿保留远端、确定=明确覆盖并同步；无冲突编辑不弹框正常提交；覆盖提交仍可撤销（10 项）。当前全部通过。
